...
+ (void)getCurrentAirquality:(id)parma result:(FlutterResult)result{
    ...
    [manager GET:requestUrl parameters:parameter headers:nil progress:nil success:^(NSURLSessionDataTask * _Nonnull task, AirBaseClass *responseObject) {
...
+ (void)getHourlyAirquality:(id)parma result:(__strong FlutterResult)result {
    NSString *latitude = parma[@"latitude"];
    NSString *longitude = parma[@"longitude"];
    BOOL isDev = parma[@"isDev"];
    NSString *requestUrl = @"";
    if(isDev){
        requestUrl = [NSString stringWithFormat:@"https://devapi.qweather.com/airquality/v1/hourly/%@/%@",latitude,longitude];
    }else{
        requestUrl = [NSString stringWithFormat:@"https://api.qweather.com/airquality/v1/hourly/%@/%@",latitude,longitude];
    }
    NSDictionary *parameter = @{@"key": parma[@"token"]};
    AFHTTPSessionManager *manager = [AFHTTPSessionManager manager];
    [manager GET:requestUrl parameters:parameter headers:nil progress:nil success:^(NSURLSessionDataTask * _Nonnull task, AirBaseClass *responseObject) {
...
+ (void)getDailyAirquality:(id)parma result:(__strong FlutterResult)result {
    NSString *latitude = parma[@"latitude"];
    NSString *longitude = parma[@"longitude"];
    BOOL isDev = parma[@"isDev"];
    NSString *requestUrl = @"";
    if(isDev){
        requestUrl = [NSString stringWithFormat:@"https://devapi.qweather.com/airquality/v1/daily/%@/%@",latitude,longitude];
    }else{
        requestUrl = [NSString stringWithFormat:@"https://api.qweather.com/airquality/v1/daily/%@/%@",latitude,longitude];
    }
    NSDictionary *parameter = @{@"key": parma[@"token"]};
    AFHTTPSessionManager *manager = [AFHTTPSessionManager manager];
    [manager GET:requestUrl parameters:parameter headers:nil progress:nil success:^(NSURLSessionDataTask * _Nonnull task, AirBaseClass *responseObject) {
